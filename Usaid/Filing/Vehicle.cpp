...
void Vehicle::deserialize(std::istream& is) {
    is >> id >> make >> model >> year >> pricePerDay >> isAvailable;
}

void Vehicle::saveToFile(const std::vector<Vehicle>& vehicles, const std::string& filename) {
...
std::vector<Vehicle> Vehicle::loadFromFile(const std::string& filename) {
